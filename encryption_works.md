...
Attacks like these become much harder as the number of nodes and the volume of traffic increases -- not impossible, but more time-consuming and expensive. Everyone can pitch in and bolster the Tor network by setting up a node, which Tor traffic can flow through. Since colleges and universities often have strong ISP infrastructure, students and staff are particularly [encouraged](https://www.eff.org/torchallenge/tor-on-campus.html) to set up a Tor node on campus. (If you're concerned about the legality of running a Tor exit node, check out the Tor Project's [legal FAQ](https://www.torproject.org/eff/tor-legal-faq.html.en).)
